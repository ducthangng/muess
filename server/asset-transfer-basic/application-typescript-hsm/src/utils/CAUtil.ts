 ...
 */
export const enrollUserToWallet = async (
  ...
  try {
    ...
    if (identity) {
      console.log(
        `Identity ${userToEnroll.userId} already exists in the wallet`
      );
      return;
    }
    ...
    console.log(
      `Successfully enrolled user ${userToEnroll.userId} and imported it into the wallet`
    );
  } catch (error) {
...
export const registerUser = async (
  ...
  try {
    ...
    const adminIdentity = await userToRegister.wallet.get(
      userToRegister.adminId
    );
    if (!adminIdentity) {
      console.log(
    ...
    const provider = userToRegister.wallet
      .getProviderRegistry()
