...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "\n--> Evaluate Transaction: ReadAsset, function returns information about an asset with ID(proposal1)"
      );
      result = await contract.evaluateTransaction("ReadAsset", proposalID);
      console.log(`*** Result: ${prettyJSONString(result.toString())}`);

      console.log(
        '\n--> Evaluate Transaction: AssetExists, function returns "true" if an asset with ID(proposal1) exist'
      );
      result = await contract.evaluateTransaction("AssetExists", proposalID);
      console.log(`*** Result: ${prettyJSONString(result.toString())}`);

      // Let's try a query operation (function).
      // This will be sent to just one peer and the results will be shown.
      // console.log('\n--> Evaluate Transaction: GetAssetsByRange, function returns assets in a specific range from proposal1 to before asset6');
      // result = await contract.evaluateTransaction('GetAssetsByRange', 'proposal1', 'asset6');
      // console.log(`*** Result: ${prettyJSONString(result.toString())}`);
      ...
      console.log(
        "\n--> Evaluate Transaction: QueryProposalsByBuyerID, find all proposal assets with buyerID(nghihua)"
      );
      ...
      console.log("accept the proposal");
      result = await contract.submitTransaction(
        "AcceptProposal",
        licenseID,
        "billgates",
      ...
      console.log(
        "\n--> Evaluate Transaction: QueryLicensesByOwnerID, find all license assets with ownerID(nghihua)"
      );
      result = await contract.evaluateTransaction(
